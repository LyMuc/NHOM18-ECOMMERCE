...
const ForgotPassword = () => {
  ...
  const [formFields, setFormsFields] = useState({
    email: localStorage.getItem("userEmail"),  // ← Lấy email từ localStorage
    newPassword: '',
    confirmPassword: ''
  });
  ...
  const handleSubmit = (e) => {
    ...
    postData(`/api/users/forgot-password/change-password`, formFields).then((res) => {
      console.log(res)
      if (res?.error === false) {
        // Xóa localStorage
        ...
        setIsLoading(false);
        history("/login")  // Chuyển về trang Login
      }
      else {
        ...
        <div className="card shadow-md w-full sm:w-[400px] m-auto rounded-md bg-white p-5 px-10">
          ...
          <form className="w-full mt-5" onSubmit={handleSubmit}>
            ...
            <div className="flex items-center w-full mt-3 mb-3">
              ...
              >
                {isLoading === true ? 
                  <CircularProgress color="inherit" /> : 'Change Password'
                }
              </Button>
            </div>
          </form>
